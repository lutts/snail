...
class KbNodeProviderTest : public ::testing::Test {
 protected:
  KbNodeProviderTest() {
...
TEST_F(KbNodeProviderTest,
  ...
  auto mock_listener = MockListener::attachTo(kbnode_provider_.get());
  {
    ...
    EXPECT_CALL(node_manager_, findKbNode(filter_pattern))
...
TEST_F(KbNodeProviderTest,
  ...
  EXPECT_CALL(node_manager_, findKbNode(_)).Times(0);
...
void KbNodeProviderTest::checkSubNodes(
    IKbNode* parent_node,
    std::map<IKbNode*, std::vector<IKbNode*> >*  kbnode_to_subnodes) {
...
void KbNodeProviderTest::checkNonFilterModelKbNodeHierarchy() {
  ...
  EXPECT_CALL(node_manager_, childNodes(_))
      .WillRepeatedly(Invoke([&kbnode_to_subnodes](IKbNode* parent_kbnode) {
...
TEST_F(KbNodeProviderTest,
  ...
  // Expectations
  EXPECT_CALL(node_manager_, addKbNode(expect_name, foo1_parent_node))
      .WillOnce(Return(foo1_child_node));
  // when called with null parent, root is the actual parent
  EXPECT_CALL(node_manager_, addKbNode(expect_name, &root_kbnode_))
      .WillOnce(Return(root_child_node));

  // Exercise system
  auto actual_foo1_child_node =
      kbnode_provider_->addKbNode(expect_name, foo1_parent_node);
  auto actual_root_child_node =
      kbnode_provider_->addKbNode(expect_name, nullptr);
...
TEST_F(KbNodeProviderTest,
  ...
  // Expectations
  EXPECT_CALL(node_manager_, findKbNode(name))
      .WillOnce(Return(expect_kbnodes));
...
TEST_F(KbNodeProviderTest,
  ...
  EXPECT_CALL(node_manager_, findKbNode(_))
