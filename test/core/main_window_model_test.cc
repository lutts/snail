...
TEST_F(MainWindowModelTest, should_get_new_title_after_setWindowTitle) { // NOLINT
  // expectations
...
TEST_F(MainWindowModelTest, should_fire_WindowTitleChanged_event_when_set_different_title) { // NOLINT
  // expectations
  utils::U8String expect_new_title =
      xtestutils::genRandomDifferentString(model->windowTitle());
...
TEST_F(MainWindowModelTest, should_not_fire_WindowTitleChanged_when_set_same_title) { // NOLINT
  // expectations
...
TEST_F(MainWindowModelTest, should_requestClose_default_return_true_when_no_listeners) { // NOLINT
  // Verify results
  ASSERT_TRUE(model->requestClose());
}

TEST_F(MainWindowModelTest, should_fire_RequestClose_when_requestClose_called) { // NOLINT
  auto tester = [this](bool expect_result) {
    // expectations
    auto mockListener = MockListener::attachTo(model.get());
    EXPECT_CALL(*mockListener, RequestClose())
...
TEST_F(MainWindowModelTest, should_and_all_listener_response_when_fire_RequestClose_event) { // NOLINT
