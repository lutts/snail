  ...
  myButton(0)
{
    ...
    myTitle->setProperty("class", QStringLiteral("header"));
...
void TaskHeader::setHasEditButton(bool has_edit_button) {
  if (has_edit_button && editButton)
    return;

  if (!has_edit_button && editButton) {
    headerLayout()->removeWidget(editButton);
    editButton->deleteLater();
    editButton = nullptr;
    return;
  }

  editButton = new QSint::ActionLabel(this);
