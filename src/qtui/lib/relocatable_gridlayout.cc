...
// this is a QGridLayout, add support to allow client modify item position after add
...
struct RelocatableGridLayoutSizeTriple
{
    QSize minS;
    QSize hint;
    QSize maxS;
...
/*
  Three internal classes related to RelocatableGridLayout: (1) QGridBox is a
  QLayoutItem with (row, column) information and (torow, tocolumn) information; (3) RelocatableGridLayoutData is
  the internal representation of a RelocatableGridLayout.
*/
...
public:
    ...
    QGridBox(const QLayout *l, QWidget *wid) { (void)l; item_ = new QWidgetItem(wid); }
...
private:
    ...
    friend class RelocatableGridLayout;
...
public:
  ...
  // custom added method begin
  void setItemPosition(int index, int row, int column, int rowSpan, int columnSpan) {
    if (index < things.count()) {
      QGridBox* box = things.at(index);
      setBoxPosition(box, row, column, rowSpan, columnSpan);
    }
  }

  void setItemPosition(QLayoutItem* item, int row, int column, int rowSpan, int columnSpan) {
  ...
  void setItemPosition(QWidget* widget, int row, int column, int rowSpan, int columnSpan) {
    for (int index = 0; index < things.count(); ++index) {
      QGridBox* box = things.at(index);
...
private:
    ...
    void addData(QGridBox *b, const RelocatableGridLayoutSizeTriple &sizes, bool r, bool c);
  ...
  // custom added method begin
  void setBoxPosition(QGridBox* box, int row, int column,
                      int rowSpan, int columnSpan) {
    if (rowSpan <= 0) {
      int oldRowSpan = box->torow - box->row + 1;
      rowSpan = oldRowSpan;
    }

    if (columnSpan <= 0) {
      int oldColumnSpan = box->tocol - box->col + 1;
      columnSpan = oldColumnSpan;
    }

    int new_torow = row + rowSpan - 1;
    int new_tocol = column + columnSpan - 1;

    expand(new_torow + 1, new_tocol + 1);
 ...
 private:
  RelocatableGridLayout* q_ptr;
  friend class RelocatableGridLayout;
};

void RelocatableGridLayoutPrivate::effectiveMargins(int *left, int *top, int *right, int *bottom) const
{
    int l = leftMargin;
    int t = topMargin;
    int r = rightMargin;
...
RelocatableGridLayoutPrivate::RelocatableGridLayoutPrivate()
{
    addVertical = false;
    setDirty();
    rr = cc = 0;
...
#if 0
RelocatableGridLayoutPrivate::RelocatableGridLayoutPrivate(int nRows, int nCols)
    : rowData(0), colData(0)
{
    init();
    if (nRows  < 0) {
...
void RelocatableGridLayoutPrivate::deleteAll()
{
...
bool RelocatableGridLayoutPrivate::hasHeightForWidth(int hSpacing, int vSpacing)
{
...
*/
void RelocatableGridLayoutPrivate::recalcHFW(int w)
{
    /*
...
int RelocatableGridLayoutPrivate::heightForWidth(int w, int hSpacing, int vSpacing)
{
    setupLayoutData(hSpacing, vSpacing);
    if (!has_hfw)
        return -1;
...
int RelocatableGridLayoutPrivate::minimumHeightForWidth(int w, int hSpacing, int vSpacing)
{
...
QSize RelocatableGridLayoutPrivate::findSize(int QLayoutStruct::*size, int hSpacing, int vSpacing) const
{
    RelocatableGridLayoutPrivate *that = const_cast<RelocatableGridLayoutPrivate*>(this);
...
Qt::Orientations RelocatableGridLayoutPrivate::expandingDirections(int hSpacing, int vSpacing) const
{
    RelocatableGridLayoutPrivate *that = const_cast<RelocatableGridLayoutPrivate*>(this);
    that->setupLayoutData(hSpacing, vSpacing);
    Qt::Orientations ret;
...
QSize RelocatableGridLayoutPrivate::sizeHint(int hSpacing, int vSpacing) const
{
    return findSize(&QLayoutStruct::sizeHint, hSpacing, vSpacing);
}

QSize RelocatableGridLayoutPrivate::maximumSize(int hSpacing, int vSpacing) const
{
    return findSize(&QLayoutStruct::maximumSize, hSpacing, vSpacing);
}

QSize RelocatableGridLayoutPrivate::minimumSize(int hSpacing, int vSpacing) const
{
    return findSize(&QLayoutStruct::minimumSize, hSpacing, vSpacing);
}

void RelocatableGridLayoutPrivate::setSize(int r, int c)
{
...
void RelocatableGridLayoutPrivate::setNextPosAfter(int row, int col)
{
    if (addVertical) {
...
void RelocatableGridLayoutPrivate::add(QGridBox *box, int row, int col)
{
    expand(row + 1, col + 1);
...
void RelocatableGridLayoutPrivate::add(QGridBox *box, int row1, int row2, int col1, int col2)
{
    if (row2 >= 0 && row2 < row1)
        qWarning("RelocatableGridLayout: Multi-cell fromRow greater than toRow");
    if (col2 >= 0 && col2 < col1)
        qWarning("RelocatableGridLayout: Multi-cell fromCol greater than toCol");
...
void RelocatableGridLayoutPrivate::addData(QGridBox *box, const RelocatableGridLayoutSizeTriple &sizes, bool r, bool c)
{
...
void RelocatableGridLayoutPrivate::setupSpacings(QVector<QLayoutStruct> &chain,
                                       ...
                                       Qt::Orientation orientation)
{
    ...
    QStyle *style = 0;
    if (fixedSpacing < 0) {
        if (QWidget *parentWidget = q_ptr->parentWidget())
                ...
                if (spacing < 0) {
                    ...
                    if (style)
                        spacing = style->combinedLayoutSpacing(controlTypes1, controlTypes2,
                                             orientation, 0, q_ptr->parentWidget());
                } else {
...
void RelocatableGridLayoutPrivate::setupLayoutData(int hSpacing, int vSpacing)
{
    ...
    int n = things.size();
    QVarLengthArray<RelocatableGridLayoutSizeTriple> sizes(n);
    ...
    */
    ...
    q_ptr->getContentsMargins(&leftMargin, &topMargin, &rightMargin, &bottomMargin);

    needRecalc = false;
}

void RelocatableGridLayoutPrivate::addHfwData(QGridBox *box, int width)
{
    QVector<QLayoutStruct> &rData = *hfwData;
    if (box->hasHeightForWidth()) {
...
*/
void RelocatableGridLayoutPrivate::setupHfwLayoutData()
{
    QVector<QLayoutStruct> &rData = *hfwData;
    for (int i = 0; i < rr; i++) {
...
void RelocatableGridLayoutPrivate::distribute(QRect r, int hSpacing, int vSpacing)
{
...
QRect RelocatableGridLayoutPrivate::cellRect(int row, int col) const
{
...
/*!
    \class RelocatableGridLayout

    \brief The RelocatableGridLayout class lays out widgets in a grid.

    \ingroup geomanagement
    \inmodule QtWidgets

    RelocatableGridLayout takes the space made available to it (by its parent
    ...
    addItem() and addWidget(). If you do this, RelocatableGridLayout will guess
    ...
    If the RelocatableGridLayout is not the top-level layout (i.e. does not
    ...
    RelocatableGridLayout also includes two margin widths:
    ...
    of the RelocatableGridLayout's four sides. The spacing() is the width of the
...
/*!
    Constructs a new RelocatableGridLayout with parent widget, \a parent.  The
    layout has one row and one column initially, and will expand when
    new items are inserted.
*/
RelocatableGridLayout::RelocatableGridLayout(QWidget *parent)
    : QLayout(parent)
    , d_ptr(utils::make_unique<RelocatableGridLayoutPrivate>())
{
  d_ptr->q_ptr = this;
  d_ptr->expand(1, 1);
}
...
*/
RelocatableGridLayout::RelocatableGridLayout()
    : QLayout(nullptr)
    , d_ptr(utils::make_unique<RelocatableGridLayoutPrivate>())
{
  d_ptr->q_ptr = this;
...
void RelocatableGridLayout::setDefaultPositioning(int n, Qt::Orientation orient)
{
    if (orient == Qt::Horizontal) {
        d_ptr->expand(1, n);
        d_ptr->addVertical = false;
    } else {
        d_ptr->expand(n,1);
        d_ptr->addVertical = true;
    }
}
...
/*!
    \property RelocatableGridLayout::horizontalSpacing
...
*/
void RelocatableGridLayout::setHorizontalSpacing(int spacing)
{
    d_ptr->horizontalSpacing = spacing;
    invalidate();
}

int RelocatableGridLayout::horizontalSpacing() const
{
...
/*!
    \property RelocatableGridLayout::verticalSpacing
...
*/
void RelocatableGridLayout::setVerticalSpacing(int spacing)
{
    d_ptr->verticalSpacing = spacing;
    invalidate();
}

int RelocatableGridLayout::verticalSpacing() const
{
    if (d_ptr->verticalSpacing >= 0) {
...
*/
void RelocatableGridLayout::setSpacing(int spacing)
{
    d_ptr->horizontalSpacing = d_ptr->verticalSpacing = spacing;
    invalidate();
}
...
*/
int RelocatableGridLayout::spacing() const
{
...
*/
int RelocatableGridLayout::rowCount() const
{
    return d_ptr->numRows();
}

/*!
    Returns the number of columns in this grid.
*/
int RelocatableGridLayout::columnCount() const
{
    return d_ptr->numCols();
}

/*!
    \reimp
*/
QSize RelocatableGridLayout::sizeHint() const
{
    QSize result(d_ptr->sizeHint(horizontalSpacing(), verticalSpacing()));
    int left, top, right, bottom;
    d_ptr->effectiveMargins(&left, &top, &right, &bottom);
...
/*!
    \reimp
*/
QSize RelocatableGridLayout::minimumSize() const
{
    QSize result(d_ptr->minimumSize(horizontalSpacing(), verticalSpacing()));
    int left, top, right, bottom;
    d_ptr->effectiveMargins(&left, &top, &right, &bottom);
    result += QSize(left + right, top + bottom);
    return result;
...
/*!
    \reimp
*/
QSize RelocatableGridLayout::maximumSize() const
{
    QSize s = d_ptr->maximumSize(horizontalSpacing(), verticalSpacing());
    int left, top, right, bottom;
    d_ptr->effectiveMargins(&left, &top, &right, &bottom);
...
/*!
    \reimp
*/
bool RelocatableGridLayout::hasHeightForWidth() const
{
    return d_ptr->hasHeightForWidth(horizontalSpacing(), verticalSpacing());
}

/*!
    \reimp
*/
int RelocatableGridLayout::heightForWidth(int w) const
{
    RelocatableGridLayoutPrivate *dat = const_cast<RelocatableGridLayoutPrivate *>(d_ptr.get());
...
/*!
    \reimp
*/
int RelocatableGridLayout::minimumHeightForWidth(int w) const
{
    RelocatableGridLayoutPrivate *dat = const_cast<RelocatableGridLayoutPrivate *>(d_ptr.get());
...
/*!
    \reimp
*/
int RelocatableGridLayout::count() const
{
    return d_ptr->count();
}
...
/*!
    \reimp
*/
QLayoutItem *RelocatableGridLayout::itemAt(int index) const
{
    return d_ptr->itemAt(index);
}
...
*/
QLayoutItem *RelocatableGridLayout::itemAtPosition(int row, int column) const
{
    int n = d_ptr->things.count();
    for (int i = 0; i < n; ++i) {
        QGridBox *box = d_ptr->things.at(i);
        if (row >= box->row && row <= box->toRow(d_ptr->rr)
                && column >= box->col && column <= box->toCol(d_ptr->cc)) {
            return box->item();
        }
    }
    return 0;
...
/*!
    \reimp
*/
QLayoutItem *RelocatableGridLayout::takeAt(int index)
{
...
*/
void RelocatableGridLayout::getItemPosition(int index, int *row, int *column, int *rowSpan, int *columnSpan) const
{
    d_ptr->getItemPosition(index, row, column, rowSpan, columnSpan);
}
...
/*!
    \reimp
*/
void RelocatableGridLayout::setGeometry(const QRect &rect)
{
    if (d_ptr->isDirty() || rect != geometry()) {
        QRect cr = alignment() ? alignmentRect(rect) : rect;
        d_ptr->distribute(cr, horizontalSpacing(), verticalSpacing());
...
*/
QRect RelocatableGridLayout::cellRect(int row, int column) const
{
    return d_ptr->cellRect(row, column);
}

/*!
    \reimp
*/
void RelocatableGridLayout::addItem(QLayoutItem *item)
{
    int r, c;
    d_ptr->getNextPos(r, c);
    addItem(item, r, c);
}
...
*/
void RelocatableGridLayout::addItem(QLayoutItem *item, int row, int column, int rowSpan, int columnSpan, Qt::Alignment alignment)
{
    QGridBox *b = new QGridBox(item);
    b->setAlignment(alignment);
    d_ptr->add(b, row, (rowSpan < 0) ? -1 : row + rowSpan - 1, column, (columnSpan < 0) ? -1 : column + columnSpan - 1);
    invalidate();
}
...
*/
void RelocatableGridLayout::addWidget(QWidget *widget, int row, int column, Qt::Alignment alignment)
{
    if (!checkWidget(this, widget))
        return;
    if (row < 0 || column < 0) {
        qWarning("RelocatableGridLayout: Cannot add %s/%s to %s/%s at row %d column %d",
...
*/
void RelocatableGridLayout::addWidget(QWidget *widget, int fromRow, int fromColumn,
                            int rowSpan, int columnSpan, Qt::Alignment alignment)
{
    ...
    b->setAlignment(alignment);
    d_ptr->add(b, fromRow, toRow, fromColumn, toColumn);
    invalidate();
}

/*!
    \fn void RelocatableGridLayout::addWidget(QWidget *widget)
...
*/
void RelocatableGridLayout::addLayout(QLayout *layout, int row, int column, Qt::Alignment alignment)
{
    if (!adoptLayout(layout))
        return;
    QGridBox *b = new QGridBox(layout);
    b->setAlignment(alignment);
    d_ptr->add(b, row, column);
}
...
*/
void RelocatableGridLayout::addLayout(QLayout *layout, int row, int column,
                                      int rowSpan, int columnSpan, Qt::Alignment alignment)
{
    if (!adoptLayout(layout))
        return;
...
*/
void RelocatableGridLayout::setRowStretch(int row, int stretch)
{
    d_ptr->setRowStretch(row, stretch);
    invalidate();
}
...
/*!
    ...
    \sa setRowStretch()
*/
int RelocatableGridLayout::rowStretch(int row) const
{
    return d_ptr->rowStretch(row);
}
...
/*!
    ...
    \sa setColumnStretch()
*/
int RelocatableGridLayout::columnStretch(int column) const
{
    return d_ptr->colStretch(column);
}
...
*/
void RelocatableGridLayout::setColumnStretch(int column, int stretch)
{
    d_ptr->setColStretch(column, stretch);
    invalidate();
}
...
*/
void RelocatableGridLayout::setRowMinimumHeight(int row, int minSize)
{
    d_ptr->setRowMinimumHeight(row, minSize);
    invalidate();
}
...
*/
int RelocatableGridLayout::rowMinimumHeight(int row) const
{
    return d_ptr->rowSpacing(row);
}
...
*/
void RelocatableGridLayout::setColumnMinimumWidth(int column, int minSize)
{
    d_ptr->setColumnMinimumWidth(column, minSize);
    invalidate();
}
...
*/
int RelocatableGridLayout::columnMinimumWidth(int column) const
{
    return d_ptr->colSpacing(column);
}

/*!
    \reimp
*/
Qt::Orientations RelocatableGridLayout::expandingDirections() const
{
    return d_ptr->expandingDirections(horizontalSpacing(), verticalSpacing());
}

/*!
    Sets the grid's origin corner, i.e. position (0, 0), to \a corner.
*/
void RelocatableGridLayout::setOriginCorner(Qt::Corner corner)
{
    d_ptr->setReversed(corner == Qt::BottomLeftCorner || corner == Qt::BottomRightCorner,
...
/*!
    ...
    position (0, 0).
*/
Qt::Corner RelocatableGridLayout::originCorner() const
{
    if (d_ptr->horReversed()) {
        return d_ptr->verReversed() ? Qt::BottomRightCorner : Qt::TopRightCorner;
    } else {
        return d_ptr->verReversed() ? Qt::BottomLeftCorner : Qt::TopLeftCorner;
    }
}

// custom method begin
void RelocatableGridLayout::setItemPosition(int index, int row, int column, int rowSpan, int columnSpan) {
  d_ptr->setItemPosition(index, row, column, rowSpan, columnSpan);
}

void RelocatableGridLayout::setItemPosition(QLayoutItem* item, int row, int column, int rowSpan, int columnSpan) {
  d_ptr->setItemPosition(item, row, column, rowSpan, columnSpan);
}

void RelocatableGridLayout::setItemPosition(QWidget* widget, int row, int column, int rowSpan, int columnSpan) {
  d_ptr->setItemPosition(widget, row, column, rowSpan, columnSpan);
}
// custom method end
...
/*!
    \reimp
*/
void RelocatableGridLayout::invalidate()
{
    d_ptr->setDirty();
    QLayout::invalidate();
}
